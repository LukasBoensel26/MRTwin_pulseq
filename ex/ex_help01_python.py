...
y = 7.1
word = "constant"
formatted_string = f"f={x:.2}*exp(-{y:.2}*t)+{word}"
...
print(array, 'shape=', array.shape)
# try the following lines, line by line
array[0, 0, :] = 1.0
# array[0, :, 0] = 2.0
# array[:, 4, 1] = 3.0
# array[:, 0, :] = 4.0
...
# %% we also use the torch library which has great parallel computing abilities
...
A = torch.ones(5) * 3
B = np.ones(5) * 5
# sometimes things need to be done in numpy or in torch,
# however these arrays are not compatible, e.g:
A - B  # B is converted to a torch tensor
# B - A  # throws an error
