...
# del_90_180 + pp.calc_duration(rf0) = pp.calc_rf_center(rf0)[0] + rf0.delay + 0.5 * adc_duration
# --> solve for adc_duration
adc_duration = 2 * (del_90_180 + pp.calc_duration(rf0) - pp.calc_rf_center(rf0)[0] - rf0.delay)
adc = pp.make_adc(num_samples=Nread, duration=adc_duration, phase_offset=0 * np.pi / 180, system=system)
...
# remove this
adc1 = pp.make_adc(num_samples=Nread, duration=140e-3, phase_offset=0 * np.pi / 180, system=system)
seq.add_block(adc1)


# introduce a certain delay between excitation and inversion pulse
# seq.add_block(pp.make_delay(del_90_180))
